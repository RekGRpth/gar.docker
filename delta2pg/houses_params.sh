...
COMMAND="$(cat <<EOF
CREATE TEMP TABLE "$REGION".tmp (LIKE "$REGION".houses_params INCLUDING ALL);
...
INSERT INTO "$REGION".houses_params SELECT "id","objectid","changeid","changeidend","typeid","value","updatedate","startdate","enddate" FROM "$REGION".tmp ON CONFLICT ON CONSTRAINT houses_params_pkey DO UPDATE SET
