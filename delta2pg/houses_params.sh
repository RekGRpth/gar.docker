...
COMMAND="$(cat <<EOF
CREATE TEMP TABLE tmp (LIKE "$REGION".houses_params INCLUDING ALL) ON COMMIT DROP;
...
INSERT INTO "$REGION".houses_params SELECT "id","objectid","changeid","changeidend","typeid","value","updatedate","startdate","enddate" FROM tmp ON CONFLICT ON CONSTRAINT houses_params_pkey DO UPDATE SET
...
DELETE FROM "$REGION".houses_params WHERE NOT current_timestamp between startdate and enddate;
EOF
)"
