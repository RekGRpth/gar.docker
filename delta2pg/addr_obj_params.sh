...
COMMAND="$(cat <<EOF
CREATE TEMP TABLE tmp (LIKE "$REGION".addr_obj_params INCLUDING ALL) ON COMMIT DROP;
...
INSERT INTO "$REGION".addr_obj_params SELECT "id","objectid","changeid","changeidend","typeid","value","updatedate","startdate","enddate" FROM tmp ON CONFLICT ON CONSTRAINT addr_obj_params_pkey DO UPDATE SET
