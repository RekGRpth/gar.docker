...
COMMAND="$(cat <<EOF
CREATE TEMP TABLE "$REGION".tmp (LIKE "$REGION".apartments_params INCLUDING ALL);
...
INSERT INTO "$REGION".apartments_params SELECT "id","objectid","changeid","changeidend","typeid","value","updatedate","startdate","enddate" FROM "$REGION".tmp ON CONFLICT ON CONSTRAINT apartments_params_pkey DO UPDATE SET
