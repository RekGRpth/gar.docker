...
COMMAND="$(cat <<EOF
CREATE TEMP TABLE tmp (LIKE "$REGION".apartments_params INCLUDING ALL) ON COMMIT DROP;
...
INSERT INTO "$REGION".apartments_params SELECT "id","objectid","changeid","changeidend","typeid","value","updatedate","startdate","enddate" FROM tmp ON CONFLICT ON CONSTRAINT apartments_params_pkey DO UPDATE SET
