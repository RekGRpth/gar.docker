...
COMMAND="$(cat <<EOF
CREATE TEMP TABLE tmp (LIKE "$REGION".change_history INCLUDING ALL) ON COMMIT DROP;
...
INSERT INTO "$REGION".change_history SELECT "changeid","objectid","adrobjectid","opertypeid","ndocid","changedate" FROM tmp ON CONFLICT ON CONSTRAINT change_history_pkey DO UPDATE SET
