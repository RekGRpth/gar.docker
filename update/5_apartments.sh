...
WITH s AS (
    ...
    SELECT s.* FROM s INNER JOIN gar AS g ON g.id = s.id AND g.object = 'apartments' AND g.region = $REGION WHERE (s.parent, s.name, s.short, s.type, s.post) IS DISTINCT FROM (g.parent, g.name, g.short, g.type, g.post) FOR UPDATE OF g SKIP LOCKED
), i AS (
    INSERT INTO gar AS g SELECT s.* FROM s LEFT JOIN gar AS g ON g.id = s.id AND g.object = 'apartments' AND g.region = $REGION WHERE g.id IS NULL RETURNING g.*, 'insert' as command
), u AS (
