...
$body$;
CREATE OR REPLACE FUNCTION gar_insert(id uuid, parent uuid, name text, short text, type text, post text) RETURNS gar_view LANGUAGE sql AS $body$
    insert into gar (id, parent, name, short, type, post)
    values (coalesce(gar_insert.id, gen_random_uuid()), gar_insert.parent, gar_insert.name, gar_insert.short, gar_insert.type, gar_insert.post)
...
CREATE OR REPLACE FUNCTION gar_select(id uuid, parent uuid DEFAULT NULL) RETURNS SETOF gar_view LANGUAGE sql STABLE AS $body$
    ...
    ) select id, parent, name, short, type, post, gar_text(name, short, type) AS text from _ order by i desc;
$body$;
CREATE OR REPLACE FUNCTION gar_select(parent uuid, name text, short text, type text, post text) RETURNS SETOF gar_view LANGUAGE sql STABLE AS $body$
...
$body$;
CREATE OR REPLACE FUNCTION gar_select_child(parent uuid, name text, short text, type text, post text) RETURNS SETOF gar_view LANGUAGE sql STABLE AS $body$
    with _ as (
        with recursive _ as (
        ...
        order by i
    ) select id, parent, name, short, type, post, gar_text(name, short, type) AS text from _ order by to_number('0'||name, '999999999'), name;
$body$;
CREATE OR REPLACE FUNCTION gar_select_parent(parent uuid, name text, short text, type text, post text) RETURNS SETOF gar_view LANGUAGE sql STABLE AS $body$
...
END;$body$;*/
CREATE OR REPLACE FUNCTION gar_update(id uuid, parent uuid, name text, short text, type text, post text) RETURNS gar_view LANGUAGE sql AS $body$
    UPDATE gar SET
