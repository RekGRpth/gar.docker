...
CREATE OR REPLACE FUNCTION gar_select(uuid uuid, parent_uuid uuid) RETURNS SETOF gar LANGUAGE sql STABLE AS $body$
    ...
    ) select uuid, parent_uuid, name, short, type, post, text from _ order by i desc;
$body$;
CREATE OR REPLACE FUNCTION gar_select(parent_uuid uuid, name text, short text, type text, post text, text text) RETURNS SETOF gar LANGUAGE sql STABLE AS $body$
...
CREATE OR REPLACE FUNCTION gar_text(uuid uuid[], post boolean DEFAULT NULL, "all" boolean DEFAULT NULL) RETURNS text LANGUAGE sql STABLE AS $body$
    with _ as (
        ...
        ) select text from p union select string_agg(text, ', ') as text from _ where type not in ('Подъезд', 'Этаж') or coalesce(gar_text.all, false)
...
$body$;
CREATE OR REPLACE FUNCTION gar_all(uuid uuid) RETURNS text LANGUAGE sql STABLE AS $body$
    select gar_text(gar_all.uuid, false, true);
$body$;
