...
begin
    ...
    if tbl ~ E'^addr_obj_\d\d$' then
        ...
        left join addr_obj_$$||local.dir||$$ as apartments_parent on apartments_parent.objectid = adm_hierarchy.parentobjid and apartments_parent.isactive = 1 and apartments_parent.isactual = 1 and current_timestamp between apartments_parent.startdate and apartments_parent.enddate
        left join param_types on param_types.name = 'Почтовый индекс' and isactive and current_timestamp between param_types.startdate and param_types.enddate
    ...
    elsif tbl ~ E'carplaces_\d\d' then
        execute $$ insert into tmp1 SELECT distinct
            carplaces.objectguid AS uuid,
            carplaces_parent.objectguid AS parent_uuid,
            carplaces.number AS name,
            'м-м' AS short,
            'Машино-место' AS type,
            carplaces_params.value AS post/*,
            socrbase.kod_t_st::integer AS level*/
        FROM carplaces_$$||local.dir||$$ as carplaces
        left join adm_hierarchy_$$||local.dir||$$ as adm_hierarchy on adm_hierarchy.objectid = carplaces.objectid and adm_hierarchy.isactive = 1 and current_timestamp between adm_hierarchy.startdate and adm_hierarchy.enddate
