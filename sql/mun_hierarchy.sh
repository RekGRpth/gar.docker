...
CREATE TABLE IF NOT EXISTS "${DIR}".mun_hierarchy (CONSTRAINT mun_hierarchy_pkey PRIMARY KEY (id)) INHERITS (mun_hierarchy);
...
CREATE INDEX IF NOT EXISTS mun_hierarchy_startdate_idx ON "${DIR}".mun_hierarchy USING btree (startdate);
CREATE INDEX IF NOT EXISTS mun_hierarchy_enddate_idx ON "${DIR}".mun_hierarchy USING btree (enddate);
